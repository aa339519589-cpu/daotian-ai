(function(){
  ...
  try{
    ...
    function ensureMessageRestoreStyles(){
      if(document.getElementById('restore-message-style')) return;
      const style=document.createElement('style');
      style.id='restore-message-style';
      style.textContent=`
        .message.assistant{justify-content:flex-start;background:transparent!important;border:0!important;box-shadow:none!important;}
        .message.assistant .assistant-text{max-width:min(860px,100%);white-space:pre-wrap;line-height:1.75;background:transparent!important;border:0!important;box-shadow:none!important;padding:2px 0;color:inherit;}
        .message.assistant .bubble{background:transparent!important;border:0!important;box-shadow:none!important;padding:2px 0;}
        .message.error{justify-content:flex-start;}
        .message.error .error-text{max-width:min(860px,100%);white-space:pre-wrap;line-height:1.65;border:1px solid rgba(255,120,120,.28);background:rgba(255,80,80,.08);border-radius:14px;padding:10px 12px;color:#ffb4b4;}
      `;
    ...
    function renderMessages(){
      ...
      box.innerHTML = msgs.map(function(m){
        ...
        if(m.role === 'error'){
          return `<div class="message error"><div class="error-text">${escapeHTML(m.content)}</div></div>`;
        }
        return `<div class="message assistant"><div class="assistant-text">${escapeHTML(m.content)}</div></div>`;
      }).join('');
