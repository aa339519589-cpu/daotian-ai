(async function(){
  ...
  var APP_READY = false;
  /* ── 记忆系统总开关 false=已下线 ── */
  var MEMORY_FEATURE_ENABLED = false;
  ...
  try{
    ...
    const slugify = DTM.slugify;
    const splitModels = DTM.splitModels;
    const makeProviderId = DTM.makeProviderId;
    const modelValuesFromProvider = DTM.modelValuesFromProvider;
    const normalizeProvider = DTM.normalizeProvider;
    const providerHasConfig = DTM.providerHasConfig;
    const normalizeProviders = DTM.normalizeProviders;
    const presetFromProvider = DTM.presetFromProvider;
    const providersToPresets = DTM.providersToPresets;
    const accessPackageKey = DTM.accessPackageKey;
    const accessPackageStatus = DTM.accessPackageStatus;
    const normalizeAccessPackage = DTM.normalizeAccessPackage;
    const accessPackageToPreset = DTM.accessPackageToPreset;
    const accessPackagesToPresets = DTM.accessPackagesToPresets;
    const providerKeyFromPreset = DTM.providerKeyFromPreset;
    const providersFromPresets = DTM.providersFromPresets;
    const legacyProviders = DTM.legacyProviders;
    const ensureSettingsShape = DTM.ensureSettingsShape;
...
@media (max-width:900px){\
  ...
  body.keyboard-open .sidebar:not(.closed){transform:translateX(-105%)!important;opacity:0!important;pointer-events:none!important;}\
    ...
    async function sendMessage(){
      ...
      if(MEMORY_FEATURE_ENABLED){ try{ if(!window.__MEMORY_V3_INIT__) MEMORY_V3.init(); }catch(_e){} }
      ...
      /* Retrieve server memories asynchronously — inject into system prompt (disabled) */
      var memoryContext = '';
      if(MEMORY_FEATURE_ENABLED){
        ...
        memoryContext = formatMemoryContext(retrievedMems);
      }
      ...
      var willExtract = MEMORY_FEATURE_ENABLED ? (loadAutoExtract() && quickExtract(text)) : false;
      ...
      /* Async memory ingest — fire-and-forget to server, never blocks chat (disabled) */
      if(MEMORY_FEATURE_ENABLED){
        try{
          var _facts = [];
          if(willExtract){
   ...
   var MEMORY_V3 = null; /* disabled: MEMORY_FEATURE_ENABLED=false */
    ...
    /* 预初始化记忆引擎（后台加载向量模型） — disabled */
    if(MEMORY_FEATURE_ENABLED) initMemoryEngine();
